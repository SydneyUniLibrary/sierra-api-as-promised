...
class SierraPatronApi_v40 {
  ...
  constructor(sierraAPIConnection, urlVersion = 'v4') {
  ...
  getMetadata({ fields, language } = {}) {
    fields = qsUtils.joinArray(fields)
    return this._connection.get(`/${this.urlVersion}/patrons/metadata`, { fields, language })
  }


  query(offset, limit, queryParameters) {
    return this._connection.post(`/${this.urlVersion}/patrons/query`, { offset, limit }, queryParameters)
  }
  ...
  _resolveCheckoutIdOrUrlToApiUrl(checkoutIdOrUrl) {
    ...
    if (this._connection.isAbsolute(str)) {
      ...
      return `/${this.urlVersion}/patrons/checkouts/${checkoutIdOrUrl}`
    }
  }
