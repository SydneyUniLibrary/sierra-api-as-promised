...
class SierraAPIConnection {
  ...
  isAbsolute(urlString) {
    return urlString.startsWith(this._baseUrlString)
  }
  ...
  /**
   * Submits a REST API request to Sierra using the DELETE HTTP method.
   *
   ...
   */
  async delete(apiUrl, qs) {
    return await this.request('DELETE', apiUrl, { qs })
  }
  ...
  /**
   * Submits a REST API request to Sierra using the PUT HTTP method.
   *
   * Ensures we have a valid oauth2 bearer token beforehand.
   *
   * @async
   ...
   * @throws If the HTTP response is not have a 2xx status.
   */
  async put(apiUrl, qs, body) {
    return await this.request('PUT', apiUrl, { qs, body })
  }
  ...
  constructor({ apiHost, apiPath, apiKey, apiSecret, apiMajorVersion }) {
  ...
  async _initialise() {
    ...
    if (this.isCompatibleWithVersion(5, 0) && (!this.apiMajorVersion || this.apiMajorVersion === 5)) {
      const { initialiseConnection_v50 } = require('./v50')
      await initialiseConnection_v50(this)
    } else if (this.isCompatibleWithVersion(4, 0) && (!this.apiMajorVersion || this.apiMajorVersion === 4)) {
      const { initialiseConnection_v40 } = require('./v40')
      await initialiseConnection_v40(this)
    } else {
      if (this.apiMajorVersion) {
        throw new Error(
          `${this.apiHost}, running Sierra ${this.about.sierraVersion}, ` +
          `is not compatible with API version ${this.apiMajorVersion}`
        )
      } else {
        throw new Error(
          `${this.apiHost}, running Sierra ${this.about.sierraVersion}, ` +
          'is not compatible with an API version that sierra-api-as-promised supports')
      }
    }
